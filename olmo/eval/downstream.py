...
class MMLU_cloze(ICLMultiChoiceTaskDataset):
    # TODO: update this comment (question)
    """MMLU creates context with "Question: QUESTION\nAnswer:" and sends the choices as continuations
           space added as prefix to each continuation
    ...
    def doc_to_text(self, doc):
        def format_example(doc, keys):
            question_prefix = ""
            if not self.mc_labels:
                question_prefix = "Complete the following statement: "  # To make context more clear
            question = question_prefix + doc["question"].strip()
            # choices = ""
            # if self.mc_labels:
            #     choices = "".join([f"{key}. {choice}\n" for key, choice in zip(keys, doc["choices"])])
            # prompt = f"{question}\n{choices}Answer:"
            cloze_prompt = f"{question} _____."  # Blank to be filled in
        ...
        if self.current_prompt is not None:
            ...
            if "inst" in self.current_prompt:
                subject = doc.get("subject").replace("_", " ")
                prefix = f"The following are fill-in-the-blank questions (with answers) about {subject}:\n\n"
            num_shots = re.findall("\\+(\\d+)", self.current_prompt)
            if num_shots:
                dev_set = self.dev_set.get(doc.get("subject"), [])
                num_shots_int = int(num_shots[0])
...
label_to_task_map = {
    ...
    "mmlu_cloze_stem_test": (MMLU_cloze, {"dataset_name": "stem", "split": "test"}),
    "mmlu_humanities_test": (MMLU, {"dataset_name": "humanities", "split": "test"}),
    "mmlu_cloze_humanities_test": (MMLU_cloze, {"dataset_name": "humanities", "split": "test"}),
    "mmlu_social_sciences_test": (MMLU, {"dataset_name": "social_sciences", "split": "test"}),
    "mmlu_cloze_social_sciences_test": (MMLU, {"dataset_name": "social_sciences", "split": "test"}),
    "mmlu_other_test": (MMLU, {"dataset_name": "other", "split": "test"}),
    "mmlu_cloze_other_test": (MMLU, {"dataset_name": "other", "split": "test"}),
    "mmlu_stem": (MMLU, {"dataset_name": "stem"}),
    "mmlu_cloze_stem": (MMLU, {"dataset_name": "stem"}),
    "mmlu_humanities": (MMLU, {"dataset_name": "humanities"}),
    "mmlu_cloze_humanities": (MMLU, {"dataset_name": "humanities"}),
    "mmlu_social_sciences": (MMLU, {"dataset_name": "social_sciences"}),
    "mmlu_cloze_social_sciences": (MMLU, {"dataset_name": "social_sciences"}),
    "mmlu_other": (MMLU, {"dataset_name": "other"}),
    "mmlu_cloze_other": (MMLU, {"dataset_name": "other"}),
