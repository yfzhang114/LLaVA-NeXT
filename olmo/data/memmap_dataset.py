...
class CombinedDataset(Dataset):
    def __init__(self, text_dataset, img_dataset, img_mask_dataset):
        self.text_dataset = text_dataset
        self.img_dataset = img_dataset
        self.img_mask_dataset = img_mask_dataset
        assert len(text_dataset) == len(img_dataset), "Datasets must be of the same length"
        assert len(text_dataset) == len(img_mask_dataset), "Datasets must be of the same length"

    def __len__(self):
        return len(self.img_dataset)

    def __getitem__(self, idx):
        sample_a = self.text_dataset[idx]
        sample_b = self.img_dataset[idx]
        sample_c = self.img_mask_dataset[idx]
